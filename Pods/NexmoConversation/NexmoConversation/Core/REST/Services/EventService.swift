...
internal struct EventService {
    ...
    internal func send(event: SendEvent, success: @escaping (EventResponse) -> Void, failure: @escaping (Error) -> Void) -> DataRequest {
        return manager
            ...
            .responseJSON(queue: manager.queue, completionHandler: { request in
                guard case .success(let response) = request.result,
                    let json = response as? Parameters,
                    let model = EventResponse(json: json) else {
