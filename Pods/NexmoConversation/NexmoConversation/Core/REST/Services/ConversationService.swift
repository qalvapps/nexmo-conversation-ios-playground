...
internal struct ConversationService {
    ...
    /// - Returns: request
    @discardableResult
    internal func all(success: @escaping ([ConversationLiteModel]) -> Void, failure: @escaping (Error) -> Void) -> DataRequest {
        return manager
            ...
            .responseJSON(queue: manager.queue, completionHandler: {
                ...
                case .failure(let error):
                    failure(NetworkError(from: $0) ?? error)
                case .success(let response):
                    guard let json = response as? [Parameters], let models = [ConversationLiteModel].from(jsonArray: json) else {
                        return failure(HTTPSessionManager.Errors.malformedJSON)
                    }
    ...
    /// - Returns: request
    @discardableResult
    internal func all(for userId: String, success: @escaping ([ConversationLiteModel]) -> Void, failure: @escaping (Error) -> Void) -> DataRequest {
        return manager
    ...
    /// Fetch a full details conversation
    ///
    /// - Parameters:
    ///   - cid: conversation
