...
internal class SyncManager {
    ...
    private var failedSyncedConversation = Set<ConversationLiteModel>()
    ...
    private func fetchConversation(for uuid: String) throws -> Conversation {
        ...
        if (error as? NetworkError)?.localizedTitle == NetworkError.Code.Conversation.notFound.rawValue,
            ...
            failedSyncedConversation.insert(conversationToDelete.data.rest)
    ...
    private func syncConversation(uuid: String) throws {
        ...
        if let conversation = cachedConversation {
        ...
        if updatesMade || conversation.data.dataIncomplete || conversation.data.requiresSync {
            ...
            conversation.data.lastUpdated = Date()
            ...
            if isNewConversation {
                ...
                events.add { self.conversationController.conversationAdded.emit(conversation) }
            }
        }
    ...
    private func syncConversationMembers(conversation: Conversation, detail: ConversationModel, events: SignalInvocations) throws {
        ...
        for item in detail.members {
            ...
            if let existingMember = existingMember {
                ...
                case .knocking: events.add { conversation.memberKnocking.emit(member) }
                default: break
                }
            }
        }
    ...
    // TODO: Move logic
    internal func save(_ conversation: ConversationLiteModel) throws -> Conversation {
        ...
        newConversation.data.lastUpdated = Date()

        try databaseManager.conversation.insert(newConversation.data)
        
        conversationController.conversations.refetch()
        
        SignalInvocations().add { self.conversationController.conversationAdded.emit(newConversation) }
    ...
    @discardableResult
    private func process(_ conversations: [ConversationLiteModel]) -> [Conversation] {
        ...
        do {
            ...
            for newConversation in conversations where !failedSyncedConversation.contains(newConversation) {
                ...
                if let existing = objectCache.conversationCache.get(uuid: newConversation.uuid) {
                    ...
                    if newConversation.sequenceNumber > existing.data.rest.sequenceNumber {
                        ...
                        existing.data.lastUpdated = Date()
                        
                        try databaseManager.conversation.insert(existing.data)
                    }
                } else {
                    let conversation = try save(newConversation)
            ...
            try databaseManager.conversation.dirtyConversations.forEach { try syncConversation(uuid: $0.rest.uuid) }
            ...
            while true {
                ...
                if let newEvent = newEvent {
                    ...
                    if let conversation = conversation {
                        ...
                        try databaseManager.conversation.insert(conversation.data) // So that the mostRecentEventIndex is updated.
                        conversation.refreshAllEventsList()
                    }
                    ...
                    if let conversation = conversation, conversation.data.requiresSync == true {
                        try syncConversation(uuid: conversation.uuid)
                    }
                }
    ...
    private func processNewEvent(conversation: Conversation?, event: Event, events: SignalInvocations, enableTypingEvents: Bool) throws {
        ...
        if let conversation = conversation {
            switch event.type {
                ...
                case .text, .image:
                    try processNewMessage(conversation: conversation, event: event, events: events)
            ...
            if event.type == .memberInvited {
                try syncConversation(uuid: event.cid)
            /* See if it's us joining our own newly created conversation. */
            } else if event.type == .memberJoined && event.id == 1 {
                try syncConversation(uuid: event.cid)
            /* Anything else is an error. */
            } else {
                Log.error(.eventProcessor, "Received non-invite/join event (" + String(event.id) + ") on unknown conversation.")
            }
        }
    }
    ...
    private func processNewMessage(conversation: Conversation, event: Event, events: SignalInvocations) throws {
        /* Check that we don't already have this in our local database - we shouldn't have because
           start index was caculated to avoid this. */
        let uuid = EventBase.uuid(from: event.id, in: conversation.uuid)
        
        if objectCache.eventCache.get(uuid: uuid) != nil {
            /* This is only a warning because a previous sync could have been interrupted after creating
               the record for this new message, but before completely the full list of events and updating
               mostRecentEventIndex. */
            Log.warn(.eventProcessor, String(format: "Duplicate message event, id = %d, on converation: %@", event.id, conversation.description))
        }
        ...
        /* Create a new record. */
        guard let newMessage = EventBase.factory(data: DBEvent(conversationUuid: conversation.uuid, event: event, seen: seen)) else { return }
        ...
        events.add { conversation.newEventReceived.emit(event) }
    }
    ...
    private func processReceiptEvent(conversation: Conversation, event: Event, events: SignalInvocations) throws {
        ...
        if let existing = objectCache.receiptCache.get(uuid: uuid) {
            if event.type == .textSeen || event.type == .textDelivered || event.type == .imageDelivered {
